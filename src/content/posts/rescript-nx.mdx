...
```
(If you are asked "What should be the project name and where should it be generated", pick libs/finance).
...
Third, we need to make sure we're set up to use ES Modules, by changing the `format` from `cjs` to `esm`.
...
Nx should be able to manage our package.json automatically for us.
