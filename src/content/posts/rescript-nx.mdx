---
title: Extending Typescript apps with Rescript using Nx
pubDate: 2023-09-22
description: 'How we can build Rescript source code into Typescript, and use it in another Typescript app.'
author: 'Phil Denhoff'
image:
...
This is the core of Nx — packge & dependency management, with some extra caching.
...
Our fix for references is **fine only for a prototype**.
You almost certainly want to come up with a better solution for longterm use.
Let me know what you come up with!

First, we want to ensure that when we build `api`, Nx builds the projects `api` depends on (or skips those builds, iff the outputs match a previously cached version for the same inputs).
...
If we don't generate that `package.json`, the build fails to import `express`.

So, **we need to ensure that our generated file can reference its imports**.
One way of doing that is by bundling all of the imports into one giant file.
Hopefully esbuild is good at trimming dead code...
...
^^TODO^^: DO YOU NEED TO `pnpm i` in the @org/finance now? We didn't add rescript yet
...
This is what our `api` application will use. However, it's not yet available as an export from our library.
...
This isn't the Nx Way™, but that won't stop us from building a prototype that we can improve later.
With it, we can call `pnpm rescript build` in the finance project, tuning what we cache and when.
...
You can run it with `pnpm nx build finance`.
...
Now we have ReScript source code, buildable by Nx, that generates TypeScript we can use in our Node.js app.
Why don't we try to call it?
